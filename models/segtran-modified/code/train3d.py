...
parser.add_argument('--task', dest='task_name', type=str,
                    default='brats', help='Name of the segmentation task.')
parser.add_argument('--ds', dest='train_ds_names', type=str, default=None,
...
parser.add_argument('--saveiter', type=int,  default=500,
                    help='save model snapshot every N iterations')

parser.add_argument('--lrwarmup', dest='lr_warmup_steps',
                    type=int,  default=500, help='Number of LR warmup steps')
parser.add_argument('--dicewarmup', dest='dice_warmup_steps', type=int,
                    default=0, help='Number of dice warmup steps (0: disabled)')
parser.add_argument('--bs', dest='batch_size', type=int,
                    default=4, help='Total batch_size on all GPUs')
parser.add_argument('--opt', type=str,  default=None,
                    help='optimization algorithm')
parser.add_argument('--lr', type=float,  default=-1, help='learning rate')
parser.add_argument('--decay', type=float,  default=-1, help='weight decay')
parser.add_argument('--gradclip', dest='grad_clip',
                    type=float,  default=-1, help='gradient clip')
parser.add_argument('--attnclip', dest='attn_clip', type=int,
                    default=500, help='Segtran attention clip')
parser.add_argument('--cp', dest='checkpoint_path', type=str,
                    default=None, help='Load this checkpoint')
...
parser.add_argument("--debug", dest='debug',
                    action='store_true', help='Debug program.')
...
parser.add_argument("--randscale", type=float, default=0,
                    help='Do random scaling augmentation.')
parser.add_argument("--affine", dest='do_affine',
                    action='store_true', help='Do random affine augmentation.')
parser.add_argument('--dropout', type=float, dest='dropout_prob',
                    default=-1, help='Dropout probability')
parser.add_argument('--modes', type=int, dest='num_modes',
                    default=-1, help='Number of transformer modes')
parser.add_argument('--modedim', type=int, dest='attention_mode_dim',
                    default=-1, help='Dimension of transformer modes')
parser.add_argument('--mod', dest='chosen_modality', type=int, default=-1,
                    help='The modality to use if images are of multiple modalities')
parser.add_argument('--focus', dest='focus_class', type=int, default=-1,
                    help='The class that is particularly predicted by the current modality (with higher loss weight)')
...
get_default(args, 'orig_input_size',    default_settings,
            None, [args.task_name, 'orig_input_size'])
get_default(args, 'orig_patch_size',    default_settings,
            None, [args.task_name, 'orig_patch_size'])
get_default(args, 'input_scale',        default_settings,
            None, [args.task_name, 'input_scale'])
get_default(args, 'D_pool_K',           default_settings, -
            1,   [args.task_name, 'D_pool_K'])
get_default(args, 'xyz_permute',        default_settings,
            None, [args.task_name, 'xyz_permute'])
get_default(args, 'chosen_modality',    default_settings, -
            1,   [args.task_name, 'chosen_modality'])
get_default(args, 'num_classes',        default_settings,
            None, [args.task_name, 'num_classes'])
...
if type(args.orig_patch_size) == int:
    args.orig_patch_size = (args.orig_patch_size,
                            args.orig_patch_size, args.orig_patch_size)
...
if type(args.orig_input_size) == int:
    args.orig_input_size = (args.orig_input_size,
                            args.orig_input_size, args.orig_input_size)
if type(args.input_scale) == str:
    args.input_scale = [float(L) for L in args.input_scale.split(",")]

args.input_patch_size = [int(args.input_scale[i] * L)
                         for i, L in enumerate(args.orig_patch_size)]
print("Orig patch: {}. Model input patch: {}".format(
    args.orig_patch_size, args.input_patch_size))

get_default(args, 'ds_class',           default_settings,
            None,   [args.task_name, 'ds_class'])
get_default(args, 'train_ds_names',     default_settings,
            None, [args.task_name, 'train_ds_names'])
...
def init_optimizer(net, max_epoch, batch_per_epoch):
    ...
    optimized_params = list(
        param for param in net.named_parameters() if param[1].requires_grad)
    ...
    optimizer_grouped_parameters = [
        ...
        {'params': low_decay_params,
            'weight_decay': args.decay * 0.1,  'lr': args.lr},
        {'params': no_decay_params,
            'weight_decay': 0.0,               'lr': args.lr},
        {'params': high_lr_params,      'weight_decay': 0.0,
            'lr': args.lr * 100},
    ]
    ...
    print0("LR Warm up: %.3f=%d iters" %
           (args.lr_warmup_ratio, args.lr_warmup_steps))
...
def save_model(net, optimizer, args, checkpoint_dir, iter_num):
    if args.local_rank == 0:
        save_model_path = os.path.join(
            checkpoint_dir, 'iter_'+str(iter_num)+'.pth')
...
if __name__ == "__main__":
    ...
    get_default(args, 'opt',            default_settings,
                None,   [args.net, 'opt'])
    get_default(args, 'lr',             default_settings, -
                1,     [args.net, 'lr', args.opt])
    get_default(args, 'decay',          default_settings, -
                1,     [args.net, 'decay'])
    get_default(args, 'grad_clip',      default_settings, -
                1,     [args.net, 'grad_clip'])

    get_default(args, 'bce_weight',     default_settings,
                None,   [args.task_name, 'bce_weight'])

    if args.binarize:
        args.bce_weight = None
    else:
        args.bce_weight = torch.tensor(args.bce_weight).cuda()
        args.bce_weight = args.bce_weight * \
            (args.num_classes - 1) / args.bce_weight.sum()

    if args.net == 'segtran':
        get_default(args, 'dropout_prob',   default_settings, -1,
                    [args.net, 'dropout_prob', args.in_fpn_layers])
    ...
    common_aug_func = iaa.Sequential(
        [
            ...
            iaa.Resize(
                {'height': tgt_height, 'width': tgt_width}),
    ...
    if args.net == 'vnet':
        net = VNet(n_channels=1, num_classes=args.num_classes,
    ...
    logging.info("{} epochs, {} itertations each.".format(
    ...
    for epoch_num in tqdm(range(start_epoch, max_epoch), ncols=70, disable=(args.local_rank != 0)):
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            volume_batch, mask_batch = sampled_batch['image'].cuda(
            ), sampled_batch['mask'].cuda()
            ...
            dice_losses = []
            # * warmup_constant(iter_num, args.dice_warmup_steps)
            ...
            if args.distributed:
                ...
                dice_losses = [reduce_tensor(dice_loss.data)
                               for dice_loss in dice_losses]
                loss = reduce_tensor(loss.data)

            if is_master:
                writer.add_scalar('lr', lr_, iter_num)
                writer.add_scalar('loss/total_ce_loss',
                                  total_ce_loss.item(), iter_num)
                writer.add_scalar('loss/total_dice_loss',
                                  total_dice_loss.item(), iter_num)
                writer.add_scalar('loss/loss', loss.item(), iter_num)
                if len(dice_losses) > 1:
                    dice_loss_str = ",".join(
                        ["%.4f" % dice_loss for dice_loss in dice_losses])
