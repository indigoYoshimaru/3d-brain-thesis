...
parser.add_argument('--ds', dest='test_ds_name', type=str,
                    default=None, help='Dataset name for test')
...
parser.add_argument('--iters', type=str,  default='11000,10000',
                    help='checkpoint iteration(s)')
parser.add_argument('--bs', dest='batch_size', type=int,
                    default=20, help='batch_size')
...
args_dict = {'trans_output_type': 'private',
             ...
             'in_fpn_scheme':     'AN',
             'out_fpn_scheme':    'AN',
             # Doesn't matter if we load a trained checkpoint.
             'use_pretrained': True,
             }
...
default_settings = {'unet':            {},
    ...
    'ds_class':    'BratsSet',
    'train_ds_names':  '2021train',
    'test_ds_name':    '2021valid',
    'chosen_modality': -1,
...
get_default(args, 'test_ds_name',       default_settings,
            None, [args.task_name, 'test_ds_name'])
    ...
    for iter_num in iter_nums:
        if args.checkpoint_dir:
            checkpoint_path = os.path.join(
                args.checkpoint_dir, 'iter_' + str(iter_num) + '.pth')
        ...
        if save_result:
            test_save_paths = []
            test_save_dirs = []
            test_save_dir = "%s-%s-%s-%d" % (args.net,
                                             args.job_name, timestamp, iter_num)
