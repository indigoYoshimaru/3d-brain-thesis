...
from models.segtran_modified.code.networks.segtran_ablation import RandPosEmbedder, SinuPosEmbedder, ZeroEmbedder, MultiHeadFeatTrans
...
bb2feat_dims = {'resnet34':  [64, 64,  128, 256,   512],
                ...
                # resibn: resnet + IBN layers
                'resibn101': [64, 256, 512, 1024, 2048],
...
class SegtranConfig:
    def __init__(self):
        ...
        # Seems to slightly improve accuracy, and reduces RAM and computation
        self.has_FFN_in_squeeze = False
        ...
        # Pooling settings
        # softmax, max, mean, or none. With [] means keepdim=True.
        self.pool_modes_feat = 'softmax'
...
class LearnedSoftAggregate(nn.Module):
    ...
    def forward(self, x, score_basis=None):
        ...
        x_aggr = (x * mode_attn_probs).sum(dim=self.group_dim,
...
class ExpandedFeatTrans(nn.Module):
    def __init__(self, config, name):
        ...
        self.first_norm_layer = nn.LayerNorm(
            self.feat_dim, eps=1e-12, elementwise_affine=True)
        ...
        if config.pool_modes_feat[0] == '[':
            ...
            # remove '[' and ']'
            self.pool_modes_feat = config.pool_modes_feat[1:-1]
        else:
        ...
        if self.config.feattrans_lin1_idbias_scale > 0:
            ...
            self.first_linear.weight.data[:self.feat_dim, :self.feat_dim] = \
                self.first_linear.weight.data[:self.feat_dim,
                                              :self.feat_dim] * 0.5 + identity_weight
    ...
    def forward(self, input_feat, attention_probs):
        ...
        mm_first_feat_fusion_3d = mm_first_feat_fusion.transpose(
            2, 3).reshape(B, M*F, U1)
...
class CrossAttFeatTrans(nn.Module):
    ...
    def forward(self, query_feat, key_feat=None, pos_biases=None):
        ...
        attention_scores = attention_scores / \
            math.sqrt(self.attention_mode_dim)  # [B0, 4, U1, U2]
        ...
        if self.training:
            ...
            if self.call_count % self.attn_diag_cycles == 0:
                print("max-attn: {:.2f}, avg-attn: {:.2f}, clamp-count: {}".format(
        ...
        if pos_biases is not None:
            ...
            attention_scores = attention_scores + \
                (self.pos_code_weight + posw_noise) * pos_biases
...
class SqueezedAttFeatTrans(nn.Module):
    def __init__(self, config, name):
        ...
        self.attractors = Parameter(torch.randn(
            1, self.num_attractors, self.in_feat_dim))
...
class SegtranFusionEncoder(nn.Module):
    def __init__(self, config, name):
        ...
        print(
            f'Segtran {self.name} Encoder with {self.num_translayers} trans-layers')
...
class LearnedSinuPosEmbedder(nn.Module):
    ...
    def forward(self, pos_normed):
        ...
        pos_embed_mix = torch.stack(
            (pos_embed_sin, pos_embed_cos), dim=3).view(pos_embed0.shape)
...
class SlidingPosBiases(nn.Module):
    def __init__(self, pos_dim, pos_bias_radius=7, max_pos_size=(100, 100)):
        ...
        print(
            f"Sliding-window Positional Biases, r: {R}, max size: {max_pos_size}")

    def forward(self, feat_shape, device):
        R = self.R
        spatial_shape = feat_shape[-self.pos_dim:]
        # [H, W, H, W] => [H+2R, W+2R, H+2R, W+2R].
        padded_pos_shape = list(
            spatial_shape) + [2*R + spatial_shape[i] for i in range(self.pos_dim)]
        ...
        if self.pos_dim == 2:
            ...
            padded_pos_biases[(all_h1s, all_w1s, all_h2s,
                               all_w2s)] = self.biases
...
class SegtranInputFeatEncoder(nn.Module):
    def __init__(self, config):
        ...
        if self.pos_code_type == 'lsinu':
            ...
            self.pos_coder = SlidingPosBiases(
                config.pos_dim, config.pos_bias_radius, config.max_pos_size)
