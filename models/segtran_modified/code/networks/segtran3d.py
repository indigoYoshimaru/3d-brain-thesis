...
class Segtran3d(SegtranInitWeights):
    def __init__(self, config):
        ...
        # Output class scores directly.
            ...
            if '2' in self.in_fpn_layers:
                ...
                self.out_conv3d = nn.Conv3d(
                    config.trans_out_dim, self.num_classes, 1)
            else:
        ...
        if self.backbone_type.startswith('i3d'):
            ...
            if self.D_pool_K > 1 and not self.bb_feat_upsize:
                if self.out_fpn_upsampleD_scheme == 'conv':
                    ...
                    out_feat_fpn_ups = out_feat_fpn_ups.view(
                        (out_feat_fpn.shape[0], self.out_feat_dim, self.D_pool_K) + out_feat_fpn.shape[2:])
                    feat_upsampleD_shape = list(out_feat_fpn_ups.shape)
                    feat_upsampleD_shape[2:4] = [
                        feat_upsampleD_shape[2] * feat_upsampleD_shape[3]]
                    out_feat_fpn = out_feat_fpn_ups.reshape(
                        feat_upsampleD_shape)
    ...
    def forward(self, batch):
        ...
        vfeat_fpn, vmask, D2, H2, W2 = self.in_fpn_forward(
            batch_base_feats, nonzero_mask)
        ...
        vfeat_fused = self.voxel_fusion(
            vfeat_fpn, voxels_pos, vmask.unsqueeze(2), xyz_shape)
        ...
        if self.do_out_fpn:
            vfeat_fused_fpn = self.out_fpn_forward(
                batch_base_feats, vfeat_fused)
        else:
