...
from models.segtran_modified.code.common_util import get_filename
import warnings
import csv
    ...
    if len(mask_onehot.shape) == 4:
        ...
        if colormap is not None:
            mask = torch.index_select(
                colormap, 0, mask.flatten()).view(list(mask.shape)+[3])
        else:
...
def fundus_map_mask(mask, exclusive=False):
    ...
    if mask.ndim == 4:
        ...
        # 0        in channel 0 is background.
        mask_nhot[:, 0] = (mask[:, 0] == 0)
        if not exclusive:
            # 1 or 255 in channel 0 is optic disc AND optic cup.
            mask_nhot[:, 1] = (mask[:, 0] >= 1)
        else:
            # 1 or 255 in channel 0, excluding 1/255 in channel 1 is optic disc only.
            mask_nhot[:, 1] = (mask[:, 0] >= 1) & (mask[:, 1] == 0)
        # 1 or 255 in channel 1 is optic cup.
        mask_nhot[:, 2] = (mask[:, 1] >= 1)
    # No batch dimension
    elif mask.ndim == 3:
        # Fake mask. No groundtruth mask available.
        if mask.shape[0] == 1:
    ...
    # Convert REFUGE official annotation format to onehot encoding.
        ...
        # 255 (white) in channel 0 is background.
        mask_nhot[0] = (mask == 255)
        if not exclusive:
            # 128 or 0 is optic disc AND optic cup.
            mask_nhot[1] = (mask <= 128)
        else:
            # 128 is optic disc only.
            mask_nhot[1] = mask[0] == 128
...
class SegCrop(Dataset):
    ...
    def __init__(self, base_dir, split, mode, sample_num=-1,
        ...
        if sample_num > 0:
            ...
            trainlist_filepath = self._base_dir + \
                '/train-{}shot.list'.format(sample_num)
            testlist_filepath = self._base_dir + \
                '/test-{}shot.list'.format(sample_num)
        ...
        if self.chosen_size:
            image_list2 = filter(lambda name: '_{}_'.format(
                self.chosen_size) in name, image_list)
        ...
        if self.chosen_size:
            ...
            print("'{}' {} samples in '{}'".format(
                self.split, self.sample_num, base_dir))
        ...
        with warnings.catch_warnings():
            ...
            if self.uncropped_size is -1:
                ...
                orig_image_path = os.path.join(
                    self._base_dir, "..", self.orig_dir, image_trunk + self.orig_ext)
    ...
    def create_file_list(self, alllist_filepath, trainlist_filepath, testlist_filepath,
        ...
        # List of file unique indices
        indices = list(self.idx2filenames.keys())
        ...
        if type(train_test_split_frac_or_shot) == int:
            ...
            train_len = int(
                np.floor(num_files * train_test_split_frac_or_shot))
        ...
        print("%d files are split to %d training, %d test" %
              (num_files, train_len, len(test_indices)))
